#ifndef __clang__
    #error "Only clang supports C2x [[attributes]] and features used in this unit!"
#endif
    ...
    switch (message) {
        case WM_CREATE :
            ...
            hwndRect  = CreateWindow(
                TEXT("static"), nullptr, WS_CHILD | WS_VISIBLE | SS_WHITERECT, 0, 0, 0, 0, hwnd, (HMENU) 9, hInstance, nullptr
            );
            ...
            for (i = 0; i < 3; i++) {
                ...
                hwndScroll[i] = CreateWindow(
                    TEXT("scrollbar"),
                    nullptr,
                    WS_CHILD | WS_VISIBLE | WS_TABSTOP | SBS_HORZ, // SBS_VERT
                    0,
                    0,
                    0,
                    0,
                ...
                hwndValue[i] = CreateWindow(
                    TEXT("static"), TEXT("0"), WS_CHILD | WS_VISIBLE | SS_CENTER, 0, 0, 0, 0, hwnd, (HMENU) (i + 6), hInstance, nullptr
                );

                OldScroll[i] = (WNDPROC) SetWindowLong(hwndScroll[i], GWLP_WNDPROC, (uintptr_t) ScrollProc);
        ...
        case WM_SETFOCUS : SetFocus(hwndScroll[idFocus]); return 0;
        ...
        case WM_VSCROLL :
            ...
            DeleteObject((HBRUSH) SetClassLong(hwnd, GCLP_HBRBACKGROUND, (uintptr_t) CreateSolidBrush(RGB(color[0], color[1], color[2]))));
        ...
        case WM_DESTROY :
            DeleteObject((HBRUSH) SetClassLong(hwnd, GCLP_HBRBACKGROUND, (uintptr_t) GetStockObject(WHITE_BRUSH)));
...
LRESULT CALLBACK ScrollProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        case WM_KEYDOWN :
            if (wParam == VK_TAB) SetFocus(GetDlgItem(GetParent(hwnd), (id + (GetKeyState(VK_SHIFT) < 0 ? 2 : 1)) % 3));
            break;

        case WM_SETFOCUS : idFocus = id; break;
    }
