...
int wmain(void) {
    ...
    _putws(L"\nV2\n");
    wprintf(L"sum is %.5Lf\n", sum_v2(UCHAR, 10, 213UI8, 146UI8, 186UI8, 106UI8, 8UI8, 224UI8, 192UI8, 12UI8, 69UI8, 169UI8));
    wprintf(L"sum is %.5Lf\n", sum_v2(CHAR, 10, 27I8, 66I8, -31I8, -15I8, 84I8, -100I8, 58I8, -128I8, 78I8, 113I8));
    wprintf(L"sum is %.5Lf\n", sum_v2(USHORT, 10, 213UI16, 146UI16, 186UI16, 106UI16, 8UI16, 224UI16, 192UI16, 12UI16, 69UI16, 169UI16));
    wprintf(L"sum is %.5Lf\n", sum_v2(SHORT, 10, 27I16, 66I16, -31I16, -15I16, 84I16, -100I16, 58I16, -128I16, 78I16, 113I16));
    wprintf(L"sum is %.5Lf\n", sum_v2(ULONG, 10, 213LU, 146LU, 186LU, 106LU, 8LU, 224LU, 192LU, 12LU, 69LU, 169LU));
    wprintf(L"sum is %.5Lf\n", sum_v2(LONG, 10, 27, 66, -31, -15, 84, -100, 58, -128, 78, 113));

    wprintf(L"sum is %.5Lf\n", sum_v2(ULONGLONG, 10, 213LLU, 146LLU, 186LLU, 106LLU, 8LLU, 224LLU, 192LLU, 12LLU, 69LLU, 169LLU));
...
static long double __declspec(noinline) __stdcall sum_v2(const type _argtype, const unsigned _argc, ...) {
    long double total = 0.000;
    static_assert(sizeof(int) == sizeof(long));           // WIN32
    static_assert(sizeof(double) == sizeof(long double)); // WIN32

    switch (_argtype) {
        // type group subject to type promotion to int (long)
        ...
        case ULONG :
            //    {
            //        const unsigned long* _ptr = NULL; // all the above unsigned types w
            //        __va_start(&_ptr, _argc);
            //        for (unsigned i = 0; i < _argc; ++i) total += _ptr[i];
            //        break;
            //    }
        ...
        case LONGLONG :
            {
                ...
                __va_start(&_ptr, _argc);
                for (unsigned i = 0; i < _argc; ++i) total += _ptr[i]; // NOLINT(cppcoreguidelines-narrowing-conversions)
                break;
            }
        case ULONGLONG :
            {
        ...
        case FLOAT : [[fallthrough]]; // floats will be promoted to doubles
        case DOUBLE :
            {
                const double* _ptr = NULL;
                __va_start(&_ptr, _argc);
