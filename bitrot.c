    ...
    */
    ...
    for (size_t i = 0; i < n; ++i) {                        // number of bits to rotate
        for (size_t j = 0; j <= (length - 1) / 8LLU; ++j) { // number of bytes in the specified bitstream
            ...
            if (!j) // first bit and last bit need swapping, so store the first bit of first byte.
                leftmost = pushoff;
            else
...
int main(void) {
    const uint8_t refbits[] = { 0, 255 };
    uint8_t       bits[]    = { 0, 255 };
    // for (size_t i = 0; i < __crt_countof(bits) * 8; ++i) wprintf_s(L"Position: %4zu, state: %2d\n", i, getbit(bits, i));

    // wprintf_s(L"Position: %4zu, state: %2d\n", i, getbit(bits, i));
    // setbit(bits, i, false);
    leftrotbit(bits, sizeof(bits) * 8, 2);
    // wprintf_s(L"Position: %4zu, state: %2d\n", i, getbit(bits, i));

    for (size_t i = 0; i < __crt_countof(bits) * 8; ++i)
        wprintf_s(L"Position: %4zu, state => original: %2d, post-rotation: %2d\n", i, getbit(refbits, i), getbit(bits, i));
    return EXIT_SUCCESS;
}
